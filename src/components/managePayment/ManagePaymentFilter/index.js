...
const schema = yup.object().shape({
 userId: yup.string(),
 paymentStatus: yup.string().oneOf(['created', 'attempted', 'paid', '']).default(''),
 action: yup.string().oneOf(['SUBSCRIPTION', 'BATCH_UPDATE']),
...
const ManagePaymentFilters = ({ handleFilter, onClose }) => {
 ...
 const { handleSubmit, control, reset, setValue, formState: { errors } } = useForm({
  ...
  defaultValues: {
   ...
   "fromDate": appliedFilters.fromDate && moment(appliedFilters?.appliedFilters.fromDate, 'DD-MM-YYYY').format('YYYY-MM-DD'),
   "action": appliedFilters.action,
   "toDate": appliedFilters.toDate && moment(appliedFilters?.appliedFilters.toDate, 'DD-MM-YYYY').format('YYYY-MM-DD'),
  }
 });
